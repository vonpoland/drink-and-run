...
export function initGame(TopDownGame) {
	...
	TopDownGame.Game.prototype = {
		...
		createPoliceman: function() {
			this.policemen = this.game.add.group();
			this.policemen.enableBody = true;
			this.policeman = this.policemen.create(112,288, 'bluecup');
		},
		update: function() {
...
//we know there is just one result
			...
			//this.police = this.game.add.sprite(32,32, 'tiles');
			//this.police.crop({ x: 0, y: 0, width: 16, height: 16});
...
//move player with cursor keys
		...
		movePlayer: function() {
			...
			this.moveState.isBlocked(this.map.getTileWorldXY(nextPosition.x, nextPosition.y, 16, 16, 'blockedLayer'));
			this.timeEvent = true;
		}
	};
}
